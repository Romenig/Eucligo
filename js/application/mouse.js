...
function grabPointForCrunstruction(event) {
    var hitResult = project.hitTestAll(event.point, grabOptions);
    if (hitResult.length == 0) {
        ...
        for (var i = 0; i < hitResult.length; i++) {
            var item = hitResult[i].item;
            if (!item.isClone) {
                if (item.toString == "EUPoint1" || item.toString == "EUPoint2" || item.toString.indexOf("troke") != -1) {
                    if (grabbedPoint != null) {
                        grabbedPoint.shadowClone.remove();
                        grabbedPoint.shadowClone = null;
                    }
                    ...
                    if (item.shadowClone == null) {
                        grabbedPoint = item;
                        currentPoints = [grabbedPoint];
                        hasGrabbed = true;
...
function moveOrDrag(event) {
    ...
    if (!isConstructing && currentBtn == 'segment') {
        if (point2 == null) {
            ...
            point2.toString = "EUPoint2";
            currentPoints.push(point2);
            tempPath = new paper.Path.Line({
                from: point1.position,
                to: point2.position,
                strokeColor: 'black'
            });
            tempPath.toString = "EUSegmentStroke";
        } else {
    ...
    if (hitResult.length == 0) {
        ...
        for (var i = 0; i < hitResult.length; i++) {
            if (selectedObjects.indexOf(hitResult[i].item) == -1) {
                if (hitResult[i].item.shadowClone == null){
